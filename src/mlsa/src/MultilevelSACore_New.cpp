...
namespace mlsa {
    SimulatedAnnealingCore_New::SimulatedAnnealingCore_New(
                const std::vector<Vertex> vertices,             // Movable vertices
                const std::vector<Edge> nets,                  // All edge connections in the selected region
        ...
        if (vertices_area_ / total_floorplan_area_ > (area_upper_bound_ - 0.05)) {
            if (vertices_area_ / total_floorplan_area_ + 0.05 <= 0.99) {
                area_upper_bound_ = vertices_area_ / total_floorplan_area_ + 0.05;
            } else {
        ...
        std::cout << "[Initialization] Number of nets : " << nets_.size() << std::endl;
    ...
    // ***************************************************************************
    // ***************************************************************************
    float SimulatedAnnealingCore_New::GetWeightCost() const {
        return weighted_cost_;
    }

    int SimulatedAnnealingCore_New::GetCrossTierNum() const {
        return num_vertical_connections_;
    }

    float SimulatedAnnealingCore_New::GetVerticalCost() const {
        return vertical_connection_cost_;
    }

    float SimulatedAnnealingCore_New::GetTierArea() const {
        return tier_area_;
    }

    float SimulatedAnnealingCore_New::GetTierWidth() const {
        return tier_width_;
    }

    float SimulatedAnnealingCore_New::GetTierHeight() const {
        return tier_height_;
    }

    float SimulatedAnnealingCore_New::GetPartitionWidth() const {
        return partition_width_;
    }

    float SimulatedAnnealingCore_New::GetPartitionHeight() const {
        return partition_height_;
    }

    float SimulatedAnnealingCore_New::GetWireLength() const {
        return wire_length_;
    }

    void SimulatedAnnealingCore_New::GetVertices(std::vector<Vertex>& vertices) const {
        vertices = vertices_;
    }

    void SimulatedAnnealingCore_New::GetPartitions(std::vector<Partition>& partitions) const {
        partitions = partitions_;
    }

    // Check whether this move is valid
    bool SimulatedAnnealingCore_New::ValidMove(const int sel_vertex, const int partition_id) const {
    ...
    bool SimulatedAnnealingCore_New::ValidSwap(const int delta_capacity, const int influenced_partition_id) const {
    ...
    void SimulatedAnnealingCore_New::InitialAssignmentScratch() {
        // Testing
    ...
    void SimulatedAnnealingCore_New::InitialAssignmentFollow() {
    ...
    void SimulatedAnnealingCore_New::InitializeSA(bool incremental_assignment) {
        // Testing, Doomed runs
        if (doom_run_) {
    ...
    void SimulatedAnnealingCore_New::RunSA() {
        ...
        while (step <= max_steps_) {
            for (unsigned int i = 0; i < num_perturb_per_step_; i++) {
                ...
                if (move_ % 25000 == 0) {
                    ...
                    // Early stop checking
                    if (std::abs(checkpoint_cost - weighted_cost_) <= 10) {
                        return;
                    }

                    // Update checkpoint
                    checkpoint_cost = weighted_cost_;
                }
            }
    ...
    void SimulatedAnnealingCore_New::CalWeightedCostScratch() {
        ...
        for (auto& sel_net : nets_) {
            // Check the status of the selected net
            if (sel_net.GetFullMovFlag()) {
                // Both vertices are movable
                int vertex_index_1 = sel_net.GetSrcIndex();
                int vertex_index_2 = sel_net.GetDstIndex();
                float net_timing_weight = sel_net.GetTimingWeight();

                float vertex_1_x_cord = vertices_[vertex_index_1].GetXCord();
                float vertex_1_y_cord = vertices_[vertex_index_1].GetYCord();
                float vertex_1_z_cord = vertices_[vertex_index_1].GetZCord();

                float vertex_2_x_cord = vertices_[vertex_index_2].GetXCord();
                float vertex_2_y_cord = vertices_[vertex_index_2].GetYCord();
                float vertex_2_z_cord = vertices_[vertex_index_2].GetZCord();

                // Delta Z
                float delta_z = std::abs(vertex_1_z_cord - vertex_2_z_cord);

                // Update the timing weight
                if (delta_z > 0) {
                ...
                // Calculate HPWL
                scratch_x_wire_length_ += net_timing_weight * std::abs(vertex_1_x_cord - vertex_2_x_cord);
                scratch_y_wire_length_ += net_timing_weight * std::abs(vertex_1_y_cord - vertex_2_y_cord);
                scratch_wire_length_ += net_timing_weight * (std::abs(vertex_1_x_cord - vertex_2_x_cord) + std::abs(vertex_1_y_cord - vertex_2_y_cord));

                // Vertical connections
                scratch_vertical_connection_cost_ += delta_z;
                num_vertical_connections_ += delta_z;
            } else {
                // This net connects movable and fixed vertices
                int vertex_index = sel_net.GetSrcIndex();
                float net_timing_weight = sel_net.GetTimingWeight();

                float vertex_x_cord = vertices_[vertex_index].GetXCord();
                float vertex_y_cord = vertices_[vertex_index].GetYCord();
                float vertex_z_cord = vertices_[vertex_index].GetZCord();

                float fixed_x_cord = sel_net.GetXCord();
                float fixed_y_cord = sel_net.GetYCord();
                float fixed_z_cord = sel_net.GetZCord();

                // Delta Z
                float delta_z = std::abs(vertex_z_cord - fixed_z_cord);
                ...
                // Calculate HPWL
                scratch_x_wire_length_ += net_timing_weight * std::abs(vertex_x_cord - fixed_x_cord);
                scratch_y_wire_length_ += net_timing_weight * std::abs(vertex_y_cord - fixed_y_cord);
                scratch_wire_length_ += net_timing_weight * (std::abs(vertex_x_cord - fixed_x_cord) + std::abs(vertex_y_cord - fixed_y_cord));
    ...
    float SimulatedAnnealingCore_New::CalAvg(std::vector<float>& value_list) {
    ...
    void SimulatedAnnealingCore_New::Perturb() {
        // Backup all metrics
        pre_weighted_cost_ = weighted_cost_;
    ...
    void SimulatedAnnealingCore_New::UpdateCostIncre(
        int sel_vertex,
        ...
        // Traverse all affected nets
        for (const auto& sel_net_index : vertices_[sel_vertex].connected_net_) {
            // Get the net
            Edge sel_net = nets_[sel_net_index];
            
            // Check which node is this net connecting to 
            if (sel_net.GetFullMovFlag()) {
                // This net is connecting a movable vertex
                int mov_vertex_id = (sel_vertex != sel_net.GetSrcIndex()) ? sel_net.GetSrcIndex() : sel_net.GetDstIndex();
                float net_timing_weight = sel_net.GetTimingWeight();
                float new_net_timing_weight = sel_net.GetTimingWeight();

                // Check whether this net is connecting to the swap target
                if (swapping_flag && (mov_vertex_id == swap_vertex_id)) {
                    continue;
                }

                // Get the movable index
                float mov_x_cord = vertices_[mov_vertex_id].GetXCord();
                float mov_y_cord = vertices_[mov_vertex_id].GetYCord();
                float mov_z_cord = vertices_[mov_vertex_id].GetZCord();
                ...
                delta_cost += (new_cost - pre_cost);
                delta_x_hpwl += (new_x_hpwl - pre_x_hpwl);
                delta_y_hpwl += (new_y_hpwl - pre_y_hpwl);
                delta_z_cost += (new_z_cost - pre_z_cost);
                ...
                // This net is connecting to a fixed vertex
                float net_timing_weight = sel_net.GetTimingWeight();
                float new_net_timing_weight = sel_net.GetTimingWeight();

                // Get the fixed vertex coordinate
                float fixed_x_cord = sel_net.GetXCord();
                float fixed_y_cord = sel_net.GetYCord();
                float fixed_z_cord = sel_net.GetZCord();
                ...
                // Calculate the original cost
                pre_cost = net_timing_weight * (std::abs(fixed_x_cord - vertex_x_cord) + std::abs(fixed_y_cord - vertex_y_cord));
                pre_x_hpwl = net_timing_weight * (std::abs(fixed_x_cord - vertex_x_cord));
                pre_y_hpwl = net_timing_weight * (std::abs(fixed_y_cord - vertex_y_cord));
                pre_z_cost = delta_z;
                ...
                // Calculate the new cost
                new_cost = new_net_timing_weight * (std::abs(fixed_x_cord - partition_x_cord) + std::abs(fixed_y_cord - partition_y_cord));
                new_x_hpwl = new_net_timing_weight * (std::abs(fixed_x_cord - partition_x_cord));
                new_y_hpwl = new_net_timing_weight * (std::abs(fixed_y_cord - partition_y_cord));
    ...
    void SimulatedAnnealingCore_New::Swap() {
        // Variable Definition
        bool continue_flag = true;
    ...
    void SimulatedAnnealingCore_New::Move() {
        // Variable definition
        bool continue_flag = true;
        float delta_cost = 0.0;
        float delta_z_cost = 0.0;
    ...
    void SimulatedAnnealingCore_New::Restore() {
